...
export class ReportsService {
  ...
  async getDemographics(query: DemographicsQueryDto) {
    ...
    try {
      ...
      const facebookAggregates = query.source === 'tiktok' ? null : {
      ...
      const tiktokAggregates = query.source === 'facebook' ? null : {
      ...
      return {
        facebook: query.source === 'tiktok' ? null : {
        ...
        tiktok: query.source === 'facebook' ? null : {
          records: tiktokDemographics.slice(0, 100),
          aggregates: tiktokAggregates
            ? {
                averageFollowers: tiktokAggregates.averageFollowers._avg.followers || 0,
                totalUniqueUsers: tiktokAggregates.totalUniqueUsers.length,
              }
            : null,
        },
      };
  ...
  private async getAgeRangeAggregates(baseWhere: Prisma.DemographicRecordWhereInput) {
    const ageRanges = [
